...
import { AddNewPlusSVG, DownloadIconSVG } from '../svgs/EmployeeListSvgs.svg';
...
const EmployeeList = () => {
  ...
  }, [currentPage, itemsPerPage, updateEmployeeList]);
  ...
  const [successEmployeeCreation, setSuccessEmployeeCreation] = useState(false);
  ...
  return (
    <DynamicSpace>
      <EmployeeListContainer>
          ...
          <EmployeeListHeadSection>
            ...
            {user && hasPermission(user, EMPLOYEE_MODULE.CREATE_EMPLOYEE) && (
              <div>
                <Button
                  className="btnMobile"
                  title={t('DOWNLOAD_OPTION_WILL_BE_AVAILABLE_SOON')}
                >
                  <DownloadIconSVG />
                  {t('DOWNLOAD')}
                </Button>
          ...
          <SideModal
            ...
            innerContainerContent={
              <CreateAccount
                ...
                handleSuccessEmployeeCreation={() =>
                  setSuccessEmployeeCreation(true)
                }
              />
            }
          />
        ...
        {successEmployeeCreation && (
          <ToastMessage
            messageType="success"
            messageBody="PROFILE_HAS_BEEN_SUCCESSFULLY_ADDED"
            messageHeading="PROFILE_ADDED"
            handleClose={() => setSuccessEmployeeCreation(false)}
          />
        )}
      </EmployeeListContainer>
    </DynamicSpace>
...
type CreateAccountProps = {
  ...
  handleSuccessEmployeeCreation: () => void;
  ...
  const handleCreateEmployee = async (e: any) => {
      ...
      email:
        formData.email === ''
          ...
          : !/^[\w-]+(\.[\w-]+)*@[\w-]+(\.[\w-]+)+$/.test(formData.email)
            ...
            : '',
      employeeId: formData.employeeId === '' ? EmployeeIDRequired : '',
    };
    ...
    if (Object.values(newErrors).every((error) => error === '')) {
      try {
        ...
        props.handleClose();
        props.handleSuccessEmployeeCreation();
      } catch (e) {
        if (axios.isAxiosError(e)) {
          const axiosError: AxiosError = e;

          const responseData: string | undefined = axiosError.response
            ?.data as string;
          ...
          if (
            ...
            setResponseMessage('EMAIL_ALREADY_FOUND');
          } else {
