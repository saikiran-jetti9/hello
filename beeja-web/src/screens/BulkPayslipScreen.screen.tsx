...
const BulkPayslip = () => {
  ...
  const handleUploadBulkPayslipSubmit = async () => {
    if (selectedFile == null || selectedFile == undefined) {
      handleToastErrorMessage('.zip File is Required');
      return;
    }
    ...
    try {
      ...
      handleToastSuccessMessage('Successfully Uploaded');
      removeFile();
      setMonthOfPayslips(
        new Date().toLocaleString('default', { month: 'long' })
      );
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (axiosError.code === 'ERR_NETWORK') {
          handleToastErrorMessage('Network Issue');
        } else if (axiosError.response?.status === 503) {
          handleToastErrorMessage('Service Unavailable');
        } else {
          handleToastErrorMessage('Upload Unsuccessful');
        }
      } else {
        handleToastErrorMessage('Upload Unsuccessful');
      }
