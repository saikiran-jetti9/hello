...
class HealthInsuranceControllerTest {
  ...
  public void testSubmitHealthInsuranceCreationException() throws Exception {
    ...
    Exception exception = assertThrows(HealthInsuranceCreationException.class, () -> {
      healthInsuranceController.submitHealthInsurance(healthInsuranceRequest, bindingResult);
    });
  ...
  public void testSubmitHealthInsuranceGeneralException() throws Exception {
    when(bindingResult.hasErrors()).thenReturn(false);
    when(healthInsuranceService.saveHealthInsurance(healthInsuranceRequest)).thenReturn(healthInsuranceResponse);
    ResponseEntity<HealthInsurance> responseEntity = healthInsuranceController.submitHealthInsurance(healthInsuranceRequest, bindingResult);
    assertEquals(HttpStatus.CREATED, responseEntity.getStatusCode());
  ...
  public void testGetHealthInsuranceByEmployeeId_Success() throws Exception {
    ...
    when(healthInsuranceService.findHealthInsuranceByEmployeeId(employeeID)).thenReturn(healthInsuranceResponse);
    ResponseEntity<HealthInsurance> responseEntity = healthInsuranceController.getHealthInsuranceByEmployeeId(employeeID);
    assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
    assertEquals(healthInsuranceResponse, responseEntity.getBody());
  }
...
public void testGetHealthInsuranceByEmployeeId_NotFound() throws Exception {
  ...
  public void testDeleteHealthInsurance_Success() throws Exception {
    ...
    when(healthInsuranceService.deleteByEmployeeIdAndOrganizationId(employeeID)).thenReturn(expectedHealthInsurance);
    ResponseEntity<HealthInsurance> response = healthInsuranceController.deleteHealthInsurance(employeeID);
  ...
  public void testDeleteHealthInsurance_NotFound() throws Exception {
    ...
    HealthInsuranceNotFoundException exception = assertThrows(HealthInsuranceNotFoundException.class, () -> {
  ...
  public void testUpdateHealthInsurance_Success() throws Exception {
    ...
    ResponseEntity<HealthInsurance> response = healthInsuranceController.updateHealthInsurance(healthInsuranceRequest, employeeID, bindingResult);
    assertEquals(HttpStatus.OK, response.getStatusCode());
